...
static uint8_t cs_enabled = 0; //1 means keep CS low
...
int SPI_init()
{
	...
	return 1;
fail:
	printf("Failed to update spi port settings\n");
	return 0;
}
/************************************************************************
* Function SPI_final()
...
int TRANS_transmitBytes(unsigned char *trBuffer, int trCount)
{
	if(trCount % 8 == 0) {
		struct spi_ioc_transfer tx = {
			...
			.len = trCount / 8,
			.cs_change = cs_enabled,
		};
		if(ioctl(fd, SPI_IOC_MESSAGE(1), &tx) < 0) return 0;
//		if(write(fd, trBuffer, trCount/8) < 0) return 0;
	} else {
		printf("Not implementd\n");
		return 0; //TODO: implement not divisable by 8
	}
	return 1;
}
...
int TRANS_receiveBytes(unsigned char *rcBuffer, int rcCount)
{
	if(rcCount % 8 == 0) {
		struct spi_ioc_transfer rx = {
			.rx_buf = (unsigned long)rcBuffer,
			.len = rcCount / 8,
			.cs_change = cs_enabled,
		};
		if(ioctl(fd, SPI_IOC_MESSAGE(1), &rx) < 0) return 0;
		//if(read(fd, rcBuffer, rcCount/8) < 0) return 0;
	} else {
...
{
	cs_enabled = 0;
	struct spi_ioc_transfer cs = {
			.cs_change = 0,
	};
	if(ioctl(fd, SPI_IOC_MESSAGE(1), &cs) < 0) return 0;
	return 1;
}
/************************************************************************
...
int TRANS_runClk()
{
	int tmpmode = mode | SPI_NO_CS;
	int ret = ioctl(fd, SPI_IOC_WR_MODE, &tmpmode);
	if(ret < 0) {
		printf("Failed to runClk\n");
		return 0;
	}

	printf("Changing spi mode to %d\n", tmpmode);
	uint8_t buff = 0xff;
	...
	ret = ioctl(fd, SPI_IOC_WR_MODE, &mode);
	if(ret < 0) {
		printf("Changing back spi mode failed\n");
	}
	return 1;
}
/************************************************************************
